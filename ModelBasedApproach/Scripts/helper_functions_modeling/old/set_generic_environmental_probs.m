...
env_inc   = r_inc * ( env_inc / sum( env_inc ) );                                    % make sum to proportion increments
env_dec   = gen_normal_tuning_skew( p_mu + (delta/2) , p_sig, LPNORM , -k, disps ); % decrement probability distribution
env_dec   = ( 1 - r_inc ) * ( env_dec / sum( env_dec ) );                            % make sum to proportion decrements
env_all   = env_inc + env_dec;  