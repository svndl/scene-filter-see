function [m] = build_model_cell_population(env,m)
%
...
elseif strcmp(m.popDensity,'optimal')
    m.pop = env.all;  end                                           % optimal cell spacing 
...
for d = 1:m.N
    ...
    [m.g_br(d),m.g_dk(d)]   = set_response_gain(env,m,m.prefs_new(d));   % gain for population type

    m.response(d,:)        = m.R * cell_response;                                                     % tuning function response ON
    %m.r_dk(d,:)        = m.R * cell_response;                                                     % tuning function response ON

end

%include a zero disparity prior assumption
%m.prior = m.R*gaussian_tuning(0,10,m.rng);
