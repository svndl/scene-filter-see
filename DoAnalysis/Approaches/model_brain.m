...
sz = [200 200]; % this should be enough to prevent cropping from the two filtering stages
%brain = cell(length(image), 1);
resp_orig = zeros(length(image), 1);
resp_enh = zeros(length(image), 1);
resp_deg = zeros(length(image), 1);
...
for x = 1:length(image)
    try
        ...
        if ~picturetrue
            ...
            interp = impaint_nans(image(x).depth, 3);
            depth = imresize(interp, sz); % with depth map
        end
