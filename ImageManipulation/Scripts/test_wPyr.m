function test_wPyr
% Function builds three pyramids(laplacian, wavelet, steerable)
% for each image/depth map in path_src, calculates depth-luminance
% correlation and mean luminance at each level (from high to low frequency). 
% For each image we generate two scatter plots and save them in path_save.


    %% check if toolbox is loaded
    if ( ~exist('buildWpyr', 'file'))
        error('matlabPyrTools is not linked, please add toolbox to the path');
    end
    
    disp('WARNING: this function requires patched reconSpyr.m. You can download it from http://hdrvdp.sourceforge.net/wiki/index.php/MatlabPyrTools_patch');
    
    %absolute/relative path
    abs_path_src = '~/Documents/MATLAB/scene-filter-see/ImageManipulation/Images/Originals';
    abs_path_save = '~/Documents/MATLAB/scene-filter-see/ImageManipulation/Images/analysis/';
    
    rel_path_src = '../Images/Originals';
    rel_path_save = '../Images/analysis';

    %figure out path locations
    if(exist(rel_path_src, 'dir'))
        path_src = rel_path_src;
        path_save = rel_path_save;
    else
        path_src = abs_path_src;
        path_save = abs_path_save;
    end
    
    if ~exist(path_save, 'dir')
        mkdir(savepath);
    end


    listing = dir(path_src);
    try
        k = 1;
        while(strcmp(listing(k).name(1),'.'))
            k = k + 1;
        end
              
        
        for l = k:length(listing)
            
            %idx = l - k + 1;
            disp(strcat('Processing ', listing(l).name));
            image_path = strcat(path_src, '/', listing(l).name);
            image = loadImages(image_path);
            im = rgb2gray(image.imRGB);
            
            %rather than cleaning the depth map, 
            %use only valid (~NaN) values for conv
             
            validIdx = ~isnan(image.imZOrig);
            depth_orig = image.imZOrig(validIdx);
            depth = image.imZOrig;
            imSubSample = 0;
            
            %laplacian pyramid
            [lpyrIm, lpindIm] = buildLpyr(im, 5 - imSubSample);
            [lpyrD, lpindD] = buildLpyr(depth, 5 - imSubSample);
            
            %wavelet pyramid
            [wpyrIm, wpindIm] = buildWpyr(im, 5 - imSubSample);
            [wpyrD, wpindD] = buildWpyr(depth, 5 - imSubSample);
            %steerable pyramid
            [spyrIm, spindIm] = buildSpyr(im, 5 - imSubSample);
            [spyrD, spindD] = buildSpyr(depth, 5 - imSubSample);
            
            %heigts 
            wH = floor(wpyrHt(wpindIm));
            lH = floor(lpyrHt(lpindIm));
            sH = floor(spyrHt(spindIm));
            
            %arrays for storing correlation values
            ld_corr_L = NaN(lH, 1);
            mean_lum_L = zeros(lH, 1);
            sf_L = zeros(lH, 1);
            
            ld_corr_W = NaN(wH, 1);
            mean_lum_W = zeros(wH, 1);
            sf_W = zeros(wH, 1);
            
            ld_corr_S = NaN(sH, 1);
            mean_lum_S = zeros(sH, 1);
            sf_S = zeros(sH, 1);
                       
            %% Original             
            sf_orig = 1/size(im, 2);            
            ld_corr_orig = corr(im(validIdx), depth_orig);           
            mean_lum_orig = mean(mean(im));

            %% Laplacian
            for lnum1 = 1:lH
                    %use reconstructed
                    lum = (reconLpyr(lpyrIm, lpindIm, lnum1));
                    depth = (reconLpyr(lpyrD, lpindD, lnum1));
                    
                    validIdx = ~isnan(depth);
                    if(~isempty(depth(validIdx)) && ...
                            ~isempty(lum(validIdx)))
                        ldcorr = corr(lum(validIdx), depth(validIdx));
                        ld_corr_L(lnum1) = ldcorr;
                    end
                    
                    mean_lum_L(lnum1) = mean(mean(normM(lum)));
                    sf_L(lnum1) = 1/lpindIm(lnum1, 2);
            end
            %% Wavelet
            for lnum2 = 1:wH
                    lum = (reconWpyr(wpyrIm, wpindIm, 'qmf9', 'reflect1', lnum2, 'all'));
                    depth = (reconWpyr(wpyrD, wpindD, 'qmf9', 'reflect1', lnum2, 'all'));

                    validIdx = ~isnan(depth);
                    if(~isempty(depth(validIdx)) && ...
                            ~isempty(lum(validIdx)))
                        ldcorr = corr(lum(validIdx), depth(validIdx));
                        ld_corr_W(lnum2) = ldcorr;
                    end
                    
                    mean_lum_W(lnum2) = mean(mean(normM(lum)));
                    sf_W(lnum2) = 1/size(wpyrBand(wpyrIm, wpindIm, lnum2), 2);
            end
            %% Steerable
            for lnum3 = 1:sH
                    lum = (reconSpyr(spyrIm, spindIm, 'sp1Filters', 'reflect1', lnum3, 'all'));
                    depth = (reconSpyr(spyrD, spindD, 'sp1Filters', 'reflect1', lnum3, 'all'));
                    
                    validIdx = ~isnan(depth);
                    if(~isempty(depth(validIdx)) && ...
                            ~isempty(lum(validIdx)))
                        ldcorr = corr(lum(validIdx), depth(validIdx));
                        ld_corr_S(lnum3) = ldcorr;                        
                    end
                    
                    mean_lum_S(lnum3) = mean(mean(normM(lum)));
                    sf_S(lnum3) = 1/size(spyrBand(spyrIm, spindIm, lnum3), 2);
            end
            
            %% display depth/luminance corr and mean luminance
            f = figure('Name', listing(l).name, 'visible', 'off');
                       
            f1 = subplot(2, 1, 1);
            title(f1, 'Luminance-depth correlation across different bands');
            xlabel(f1, 'spatial frequency');
            ylabel(f1, 'Correlation value');
            
            validL = ~isnan(ld_corr_L);
            validW = ~isnan(ld_corr_W);
            validS = ~isnan(ld_corr_S);
            scatter(sf_orig, ld_corr_orig, 'Marker', '^'); hold on;
            scatter(sf_L(validL), ld_corr_L(validL), 'Marker', '*'); hold on;
            scatter(sf_W(validW), ld_corr_W(validW), 'Marker', 'o'); hold on;
            scatter(sf_S(validS), ld_corr_S(validS), 'Marker', '+'); hold on;
            legend('Origional', 'Laplacian', 'Wavelet', 'Steerable');
        
                       
            f2 = subplot(2, 1, 2);
            title(f2, 'Mean luminance across different bands');
            xlabel(f2, 'spatial frequency');
            ylabel(f2, 'Mean');

            scatter(sf_orig, mean_lum_orig, 'Marker', '^'); hold on;
            scatter(sf_L, mean_lum_L, 'Marker', '*'); hold on;
            scatter(sf_W, mean_lum_W, 'Marker', 'o'); hold on;
            scatter(sf_S, mean_lum_S, 'Marker', '+'); hold on;
            legend('Original', 'Laplacian', 'Wavelet', 'Steerable');
            saveas(f, strcat(path_save, listing(l).name), 'jpeg');
            close(f);               
        end
    catch err
        disp(strcat('Test_wPyr:Error loading ', path));
        disp(err.message);
        disp(err.cause);
        disp(err.stack(1));
        disp(err.stack(2));
    end
    disp('done');    
 end