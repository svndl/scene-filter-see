...
for f = 1:length(fltrs)
    half_filter = floor(0.5*size(fltrs(1).ON.cntr));
    padded = padarray(im, half_filter);
    imdog       = apply_dog_filter(padded, fltrs(f).(path).cntr, fltrs(f).(path).sur);  % apply the normalized DOG
