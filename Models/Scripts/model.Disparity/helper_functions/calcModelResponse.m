function brain = calcModelResponse(model, image)
%function calculates model of visual depth perception
    ...
    if (nx <= blockSize && ny <= blockSize)
        ...
        for i = 1:h
            for j = 1:w
                display(['Calculating... ' num2str(w*(i - 1) + j) 'th block of ' num2str(w*h)]);
...
function brain_disparity = calc_chunk(model, img, disp)
    ...
    for j = 1:ny
        ...
        val2d(isnan(val2d)) = 0;
        bd = single(wmean(repmat(model.env.rng', [1 nx]), val2d));
