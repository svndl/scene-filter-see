function im_rgc = model_calcRGC(im, arcmin_px)
...
function im = calc_rgc(img, sigma)
    ...
    cen = model_mkGaussianRF(frange, sigma);                   % central gaussian
    sur = model_mkGaussianRF(frange, SUR_SCALE*sigma);         % surround gaussian
    ada = model_mkGaussianRF(frange, ADA_SCALE*sigma);         % adaptation gaussian, equal to surround
